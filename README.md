...
Proxy Protocol Support is a Forge mod that preserves clients’ real IPs when a Minecraft server is behind HAProxy/TCPShield/Nginx. The mod exposes the Proxy Protocol payload to plugins so downstream code can read the original `SocketAddress` instead of the proxy one.
...
- Proxy configuration lives in `gradle.properties`; CI and the build script propagate those system props before any module runs, so you do not need to spray proxies in multiple files.
