...
public class Configuration {
    ...
    public static Config loadConfig(File configDir) throws IOException {
        final File file = new File(configDir, ProxyProtocolSupport.CONFIG_FILE_NAME);
        ProxyProtocolSupport.infoLogger.accept("Loading configuration from " + file);

        if (file.exists()) {
            Config loadedConfig = loadConfigFile(file);
            return saveConfig(file, loadedConfig);
        }
